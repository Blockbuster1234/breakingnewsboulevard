...
// Function to fix a post string: remove ** and replace actual newlines in single/double quoted strings with \n
function fixPostString(content) {
  let result = '';
  let i = 0;
    ...
    try {
      ...
      for (const postStr of posts) {
        if (extractSlug(postStr) === slug) {
          // Fix the post string
          const fixed = fixPostString(postStr);
